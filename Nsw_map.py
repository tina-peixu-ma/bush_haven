...
nsw_df = pd.DataFrame(data, columns=columns)
...
# Create a map centered around NWS, Australia
m = folium.Map(location=[-32.17, 147.0], zoom_start=7)
...
for index, row in nsw_df.iterrows():
...
<div style="position: fixed; bottom: 50px; right: 50px; z-index: 1000; background-color: transparent; border: 2px solid black; padding: 3px; max-height: 200px; overflow-y: auto;">
     ...
     <p><i class="fa fa-circle fa-lg" style="color:olive"></i> Herb</p>
     <p><i class="fa fa-circle fa-lg" style="color:lightblue"></i> Fruit</p>
     <p><i class="fa fa-circle fa-lg" style="color:darkgreen"></i> Tree</p>
</div>
'''
...
map_file = "/var/www/html/wordpress/wp-content/themes/Nsw_plants_map.html2" 
m.save(map_file)
