...
columns = ['Plant_ID', 
    ...
    'freshwater_wetland', 
    'saltwater_wetland', 
    'most_likely_occurrence', 
    'epbc_category_of_threat', 
    'ffg_extinction_risk', 
    'ffg_category_of_threat', 
    'Latitude', 
    'Longitude', 
    'REGIONS']
vic_df = pd.DataFrame(data, columns=columns)
...
m = folium.Map(location=[-36.85, 144.3], zoom_start=7)
...
for index, row in vic_df.iterrows():
    lat = float(row['Latitude']) + np.random.uniform(-1.0, 0.8)  
    lon = float(row['Longitude']) + np.random.uniform(-2.2, 2.2)  
...
<div style="position: fixed; bottom: 50px; right: 50px; z-index: 1000; background-color: transparent; border: 2px solid black; padding: 3px; max-height: 200px; overflow-y: auto;">
     ...
     <p><i class="fa fa-circle fa-lg" style="color:cyan"></i> Climber</p>
     ...
     <p><i class="fa fa-circle fa-lg" style="color:olive"></i> Perennial</p>
     <p><i class="fa fa-circle fa-lg" style="color:yellow"></i> Herb </p>
</div>
'''
m.get_root().html.add_child(folium.Element(legend_html))
# Save the map as an HTML file
map_file = "/var/www/html/wordpress/wp-content/themes/VIC_plants_map.html" 
m.save(map_file)
